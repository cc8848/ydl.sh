...
UPDATE(){
	...
	if [ ! -s "$TMP_YTB" ]; then
		echo -e "\033[41;37myoutube-dl download failed\033[0m!  http://rg3.github.io/youtube-dl/download.html"
		exit 1
	fi
...
USAGE(){
	...
	cat <<- END
	(1) ydl.sh http://v.youku.com/v_show/id_XMTQzMTY3MjE0OA==.html
